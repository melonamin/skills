...
print the workspace/session tree (`--json` for structured). Each session node carries
...
`new [name] [--collapsed]` (`--collapsed` creates it closed in the sidebar so you can fill
...
`new [name] [--minimized]` · `list` · `select <id>` · `close <id>` · `rename <id> <name>` ·
...
`zoom [show|hide|toggle] [--target surface:<session-id>:left|right|scratch|overlay|overlay-left|overlay-right|quick] [--window W]`
...
`dashboard <ids…> [--font-size N | --auto-size] [--window W]` opens a view-only grid
...
`[show|hide|toggle]` (visibility; read back from the tree's `quickVisible`) ·
...
`[show|hide|toggle]` (visibility; read back from the tree's `sidebarVisible`) ·
...
`restore clear` — clear every session's saved foreground command (the
